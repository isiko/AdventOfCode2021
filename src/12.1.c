...
void markWay(int markedFields[], int startIndex, char* wayDescription){
    ...
    if(startIndex == getCave("end")){
    ...
    } else if(markedFields[startIndex] == 0 || caves[startIndex].size > 0){
        // printf("Marking Node\n");
        int tempMarkedFields[inputHeight] = {0};
        ...
        for (int i = 0; i < caveAmount; i++)
            if(i != startIndex && (tempMarkedFields[i] == 0 || caves[i].size > 0) && caves[startIndex].connections[i] == 1) markWay(tempMarkedFields, i, wayDescription);
    } else {
        return;
    }
}
...
int main() {
    ...
    if(readFile()){
        ...
        markWay(startingMarkedFields, getCave("start"), "");
        printf("Ways: %d", ways);
        return 0;
    }
