...
int readFile()
{
    ...
    for (int h = 0; h < inputHeight2; h++)
    {
        ...
        if (strcmp("x", currentLine[0])) folds[h][0] = 1;
        if (strcmp("y", currentLine[0])) folds[h][0] = 0;

        printf("%s-Axis corresponds to Dimension %d\n", currentLine[0], folds[h][0]);

            folds[h][1] = atoi(currentLine[1]);
...
int main()
{
    ...
    {
        printDots();
        for(int i = 0; i<inputHeight2; i++)
            foldDim(folds[i][1], folds[i][0]);
